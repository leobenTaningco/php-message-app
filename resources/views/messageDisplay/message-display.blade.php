  ...
  <div class="bg-white m-3 rounded-md shadow-lg w-80 break-inside-avoid" >
    <!-- Message Info -->
        ...
        <div class="flex flex-col flex-grow py-2">
            <div class="message-bubble relative p-3 mx-5 mb-1 text-slate-800 rounded-lg max-w-[70%] self-start break-words break-all" data-color="{{ $message->color }}">
                <div class="flex flex-grow items-start">
                    <img src="{{ $message->user && $message->user->profile_picture ? asset('storage/' . $message->user->profile_picture) : 'https://pbs.twimg.com/media/GkcZw0lWEAAuGGz?format=png&name=small' }}"
                        class="w-6 h-6 rounded-full border border-gray-300 object-cover mr-1">
                    <span class="font-light text-sm text-blue-900 mb-1 ">{{ $message->user->name ?? 'Anonymous' }}</span>
                </div>
        ...
        <div class="reply-bubble relative p-3 mx-5 my-1 bg-red-200 text-slate-800 rounded-lg max-w-[70%] break-words break-all 
          ...
          <img src="{{ $reply->user && $reply->user->profile_picture ? asset('storage/' . $reply->user->profile_picture) : 'https://pbs.twimg.com/media/GkcZw0lWEAAuGGz?format=png&name=small' }}"
              class="w-6 h-6 rounded-full border border-gray-300 object-cover mr-1">
          <span class="font-light text-sm text-blue-900 mb-1 ">{{ $reply->user->name ?? 'Anonymous' }}</span>
            </div>
